    ...
    _createClass(HeaderSection, [{
      ...
      value: function _adjustDropdownPosition(event, target) {
        if (window.innerWidth <= 768) {
          return; // Don't adjust dropdowns on mobile
        }
        ...
        fastdom.mutate(function () {
          nestedMenus.forEach(function (item) {
            item.classList.toggle('DropdownMenu--reversed', shouldOpenLeft);
          });
        });
      }
      ...
      value: function _handleTouchMenu(event, target) {
        if (!target.classList.contains('is-expanded')) {
          ...
          fastdom.mutate(() => {
            const expandedItems = this.element.querySelectorAll('.is-expanded');
            expandedItems.forEach(item => {
              if (item !== target) {
                item.classList.remove('is-expanded');
                const submenu = item.querySelector('[aria-hidden]');
                if (submenu) {
      ...
      value: function _verifyNavigationOverlap() {
        ...
        fastdom.measure(() => {
          const mainTopMenu = self.element.querySelector('.Header__MainNav');
          if (mainTopMenu) {
            const firstNavElement = mainTopMenu.querySelector('.HorizontalList__Item');
            if (firstNavElement) {
              const firstNavElementHeight = __WEBPACK_IMPORTED_MODULE_0__helper_Dom__["default"].outerHeightWithMargin(firstNavElement);
              const mainNavHeight = mainTopMenu.scrollHeight;
              isOverlapping = mainNavHeight > firstNavElementHeight;
            }
          }
        });
        ...
        fastdom.mutate(() => {
          ...
          self.element.classList.remove('Header--logoLeft', 'Header--inline', 'Header--center');
          
          if (window.innerWidth <= 768) {
            // Force mobile layout
            self.element.classList.add('Header--mobile');
          } else {
            // Apply desktop layout
            self.element.classList.add(isOverlapping ? 'Header--center' : `Header--${self.options['navigationStyle']}`);
          }

          // Update header height CSS variable
          document.documentElement.style.setProperty('--header-height', `${self.element.clientHeight}px`);
        });
      }
...
document.addEventListener("DOMContentLoaded", function() {
    if (typeof gsap !== 'undefined') {
        gsap.to(".parallax-spectrum", {
            yPercent: -30,
            ease: "none",
            scrollTrigger: {
                trigger: ".parallax-spectrum",
                start: "top bottom",
                end: "bottom top",
                scrub: true
            }
