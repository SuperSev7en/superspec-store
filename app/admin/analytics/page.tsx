              ...
              {FUNNEL.map((step, i) => {
                ...
                return (
                  <div key={step.stage} className="flex items-center gap-4">
                    ...
                    {i > 0 && (
                      <span className="text-xs text-red-500 font-medium w-14 text-right">
                        -{dropOff}%
                      </span>
                    )}
                  </div>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-3 py-1 text-xs rounded-full ${sortBy === "revenue" ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-600"}`}
              >
                By Revenue
              </button>
              <button
                onClick={() => setSortBy("units")}
                className={`px-3 py-1 text-xs rounded-full ${sortBy === "units" ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-600"}`}
              >
                By Units
              </button>
              <button
                onClick={() =>
                  exportView("top_products", [
                    "Product,Units,Revenue",
                    ...sortedProducts.map(
                      (p) => `"${p.name}",${p.units},${p.revenue}`,
                    ),
                  ])
                }
      ...
      {tab === "geography" && (
        <div className="bg-white shadow rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            ...
            <button
              onClick={() =>
                exportView("geography", [
                  "Region,Orders,Revenue",
                  ...GEO_DATA.map(
                    (g) => `"${g.region}",${g.orders},${g.revenue}`,
                  ),
                ])
              }
          ...
          <table className="min-w-full divide-y divide-gray-100">
            <thead className="bg-gray-50">
              <tr>
                {["Region", "Orders", "Revenue", "% of Total"].map((h) => (
                  <th
                    key={h}
                    className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase"
                  >
              ...
              {GEO_DATA.map((g) => (
                <tr key={g.region} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-3 text-sm text-gray-500">
                    {Math.round(
                      (g.orders / GEO_DATA.reduce((s, x) => s + x.orders, 0)) *
                        100,
                    )}
                    %
                  </td>
                ...
                <div key={d.device}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-gray-500">
                      {d.sessions.toLocaleString()} ({d.pct}%)
                    </span>
                  </div>
                  <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div
