  ...
  const exportCSV = () => {
    const csv = ["Name,Email,Orders,Total Spend,Joined"]
      .concat(
        filtered.map(
          (c) =>
            `"${c.name}",${c.email},${c.orders_count},${c.total_spend.toFixed(2)},${new Date(c.created_at).toLocaleDateString()}`,
        ),
      )
      .join("\n");
          ...
          <div className="lg:col-span-1 bg-white shadow rounded-lg p-6 space-y-4">
            ...
            <div className="border-t border-gray-100 pt-4 space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold">{selected.orders_count}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Lifetime value</span>
                <span className="font-semibold">
                  ${selected.total_spend.toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between text-sm">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <p className="text-sm text-gray-500">
                No orders to display. In production, fetch from the orders table
            ...
            <div className="bg-white shadow rounded-lg p-6">
              ...
              />
              <button className="mt-2 px-4 py-2 bg-gray-900 text-white text-sm rounded-lg hover:bg-gray-800">
                Save Notes
              </button>
            </div>
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {(selected.tags || ["VIP", "Repeat Buyer"]).map((t) => (
                  <span
                    key={t}
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              {["Customer", "Email", "Orders", "Total Spend", "Joined", ""].map(
                (h) => (
                  <th
                    key={h}
