  ...
  return password.length > 0 ? (
    <div style={{ marginTop: 8 }}>
      <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
        {[1,2,3,4,5].map(i => <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i <= score ? colors[score] : 'var(--border-color)', transition: 'background 0.3s' }} />)}
      </div>
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40 }}>
        <div style={{ textAlign: 'center', maxWidth: 400 }}>
          <h1 className="Heading u-h3" style={{ marginBottom: 16 }}>Invalid Reset Link</h1>
          <p className="Text--subdued" style={{ marginBottom: 24 }}>This reset link is invalid or has expired.</p>
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px', background: 'var(--background)' }}>
      <div style={{ width: '100%', maxWidth: 440 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <p className="Text--subdued" style={{ marginTop: 8 }}>Set a new password</p>
        </div>
        ...
        <div style={{ background: 'var(--secondary-elements-background)', border: '1px solid var(--border-color)', borderRadius: 8, padding: 40 }}>
          {error && (
            <div style={{ background: 'rgba(220,38,38,0.08)', border: '1px solid rgba(220,38,38,0.3)', color: '#dc2626', padding: '12px 16px', borderRadius: 6, marginBottom: 20, fontSize: 14 }}>
              {error}{' '}
              {error.includes('expired') && <Link href="/forgot-password" className="Link Link--underline">Request a new link</Link>}
            </div>
          )}

          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: 20 }}>
              <label className="Heading u-h7" style={{ display: 'block', marginBottom: 8 }}>New Password</label>
              <div style={{ position: 'relative' }}>
                <Lock style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', width: 18, height: 18, color: 'var(--text-light-color)' }} />
                <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} className="Form__Input" style={{ paddingLeft: 40, paddingRight: 40 }} required minLength={8} autoFocus />
            ...
            <div style={{ marginBottom: 28 }}>
              <label className="Heading u-h7" style={{ display: 'block', marginBottom: 8 }}>Confirm New Password</label>
              <div style={{ position: 'relative' }}>
                <Lock style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', width: 18, height: 18, color: 'var(--text-light-color)' }} />
                <input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="Form__Input" style={{ paddingLeft: 40 }} required />
              </div>
            </div>
