...
export function Slideshow({
  ...
  const buttonBackground = typeof settings.button_background === 'string' ? settings.button_background : '#ffffff';
  ...
  const slides = blockOrder
    .map((blockId, idx) => {
      const block = blocks[blockId] as { settings?: Record<string, unknown> } | undefined;
      const s = block?.settings ?? {};

      const image = typeof s.image === 'string' ? resolveShopifyAssetUrl(s.image) : null;
      const mobileImage = typeof s.mobile_image === 'string' ? resolveShopifyAssetUrl(s.mobile_image) : null;
      const applyOverlay = Boolean(s.apply_overlay ?? false);
      const contentPosition = typeof s.content_position === 'string' ? s.content_position : 'bottomLeft';
      ...
      const b1Link = typeof s.button_1_link === 'string' ? resolveShopifyAssetUrl(s.button_1_link) : '';
      const b2Text = typeof s.button_2_text === 'string' ? s.button_2_text : '';
      const b2Link = typeof s.button_2_link === 'string' ? resolveShopifyAssetUrl(s.button_2_link) : '';
      ...
      const slideProps = isClickableImage ? ({ href: b1Link || '#' } as const) : ({} as const);
        ...
        >
          ...
          {image ? (
            <div
              ...
              style={{
                ...(showFullscreen ? {} : { paddingBottom: '44.44%', ['--aspect-ratio' as any]: 2.25 }),
            ...
            <div className={`Slideshow__ImageContainer ${applyOverlay ? 'Image--contrast' : ''} PlaceholderSvg--dark`.trim()}>
              <div className="Slideshow__Image PlaceholderBackground__Svg" />
            </div>
          )}

          {hasContent ? (
            <div className={`Slideshow__Content Slideshow__Content--${contentPosition}`.trim()}>
              <header className="SectionHeader">
                {subheading ? <h3 className="SectionHeader__SubHeading Heading u-h6">{subheading}</h3> : null}
                {title ? <h2 className="SectionHeader__Heading SectionHeader__Heading--emphasize Heading u-h1">{title}</h2> : null}
                {b1Text || b2Text ? (
                  <div className="SectionHeader__ButtonWrapper">
                    <div className={`ButtonGroup ButtonGroup--spacingSmall ${b1Text && b2Text ? 'ButtonGroup--sameSize' : ''}`.trim()}>
                      {b1Text ? (
                        <a href={b1Link || '#'} className="ButtonGroup__Item Button">
                          {b1Text}
                        </a>
                      ) : null}
                      {b2Text ? (
                        <a href={b2Link || '#'} className="ButtonGroup__Item Button">
                          {b2Text}
                        </a>
                      ) : null}
                    </div>
...
`;
  ...
  return (
    <>
      <section id={`section-${id}`} data-section-id={id} data-section-type="slideshow">
        <div className={`Slideshow ${showFullscreen ? 'Slideshow--fullscreen' : ''}`.trim()}>
          <div
          ...
          {showArrow ? (
            <button data-href={`#section-${id}-end`} className="Slideshow__ScrollButton RoundButton RoundButton--medium" aria-label="Scroll to content">
              ↓
            </button>
          ) : null}
        </div>
