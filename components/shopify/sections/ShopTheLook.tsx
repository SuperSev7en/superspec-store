...
export async function ShopTheLook({
  ...
  const showMobileProductInfo = Boolean(settings.show_mobile_product_info ?? false);
  ...
  for (const bid of blockOrder) {
    ...
    for (let i = 1; i <= 3; i++) {
      const h = typeof s[`product_${i}`] === 'string' ? String(s[`product_${i}`]).trim() : '';
    ...
    const products = handles.map((h) => byHandle.get(h)).filter(Boolean) as CatalogProduct[];
    const onboarding = !image && products.length === 0;

    looks.push({ blockId: bid, image, dotStyle, products: onboarding ? [] : products, positions });
  }

  const firstProduct = looks.flatMap((l) => l.products)[0];
  const firstHref = firstProduct ? `/products/${firstProduct.handle}` : '/products';

  return (
    <section className="Section Section--spacingNormal" data-section-id={id} data-section-type="shop-the-look">
      {subheading || title ? (
          ...
          return (
            ...
            >
              <div className="ShopTheLook__Inner">
                <div className="ShopTheLook__ImageWrapper" style={{ maxWidth: 550, position: 'relative' }}>
                  <div
                    className={look.image ? 'AspectRatio AspectRatio--withFallback' : ''}
                    style={{ minHeight: 360, background: look.image ? `url(${look.image}) center/cover` : undefined }}
                  >
                    {!look.image ? (
                      <div className="PlaceholderSvg PlaceholderSvg--dark" style={{ minHeight: 360, opacity: 0.5 }} />
                    ) : null}
                  </div>
                  {dots.map((d, i) => (
                    <a
                      ...
                      style={{ position: 'absolute', left: `${d.left}%`, top: `${d.top}%`, transform: 'translate(-50%, -50%)' }}
                ...
                >
                  {onboarding || look.products.length === 0
                    ? [0, 1, 2].map((i) => (
                        <div key={i} className={`ShopTheLook__ProductItem Carousel__Cell ${i === 0 ? 'is-selected' : ''}`.trim()}>
                          <div className="ProductItem">
                            <div className="ProductItem__Wrapper">
                              <span className="ProductItem__ImageWrapper" style={{ cursor: 'default' }}>
                                <div className="ProductItem__Image PlaceholderSvg" style={{ minHeight: 200, background: 'rgba(255,255,255,0.06)' }} />
                              </span>
                              <div className="ProductItem__Info ProductItem__Info--center">
                                ...
                                <div className="ProductItem__PriceList Heading">
                                  <span className="ProductItem__Price Price Text--subdued">$45</span>
                                </div>
                              </div>
                            </div>
                          </div>
                    ...
                    : look.products.map((p, i) => (
                        <div key={p.handle} className={`ShopTheLook__ProductItem Carousel__Cell ${i === 0 ? 'is-selected' : ''}`.trim()}>
                          <ProductItem product={p} showLabels={false} />
                        </div>
                      ))}
                  <a href={firstHref} className="ShopTheLook__ViewButton Button Button--primary Button--full">
                    View product
                  </a>
                </div>
              </div>
