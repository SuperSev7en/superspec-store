...
export async function FeaturedProduct({
  ...
  const showHeadingsOnMobile = Boolean(settings.show_headings_on_mobile ?? true);
  const handle = typeof settings.product === 'string' ? settings.product.trim() : '';
  const showVendor = Boolean(settings.show_vendor ?? false);
  const showDescription = Boolean(settings.show_description ?? true);
  const descriptionBelowAtc = Boolean(settings.description_below_add_to_cart ?? false);
      ...
      <div className="Container">
        ...
        >
          {product && variant ? (
            <>
              {product.images[0] ? (
                <a href={`/products/${product.handle}`} className="FeaturedProduct__Gallery">
                  <ProductImageFrame
              ...
              <div className="FeaturedProduct__Info">
                <div className="ProductMeta">
                  {showVendor && product.vendor ? (
                    <h2 className="ProductMeta__Vendor Heading u-h6">{product.vendor}</h2>
                  ) : null}
                  <h1 className="ProductMeta__Title Heading u-h2">{product.title}</h1>
                  <div className="ProductMeta__PriceList Heading">
                    <span className="ProductMeta__Price Price Text--subdued u-h4">${variant.price.toFixed(2)}</span>
                  </div>
                  {showDescription && !descriptionBelowAtc && product.descriptionHtml ? (
                ...
                <div className="ProductForm">
                  <AddToCartButton
                    ...
                    label={
                      showPriceInButton ? `Add to cart — $${variant.price.toFixed(2)}` : 'Add to cart'
                    }
                  />
                </div>
                ...
                <div className="FeaturedProduct__ViewWrapper">
                  <a href={`/products/${product.handle}`} className="Link Link--underline">
                    View product
                  </a>
                </div>
              </div>
            ...
            <>
              <div className="FeaturedProduct__Gallery">
                <div
                  ...
                  style={{ minHeight: 280, background: 'rgba(255,255,255,0.06)' }}
                />
              </div>
              <div className="FeaturedProduct__Info">
                <div className="ProductMeta">
                  {showVendor ? <h2 className="ProductMeta__Vendor Heading u-h6">Vendor</h2> : null}
                  <h1 className="ProductMeta__Title Heading u-h2">Sample product</h1>
                  <div className="ProductMeta__PriceList Heading">
                    <span className="ProductMeta__Price Price Text--subdued u-h4">$45</span>
                  </div>
                  {showDescription && !descriptionBelowAtc ? (
                    <div className="ProductMeta__Description Rte">
                      <p>Add a product handle in theme settings to feature a real catalog item.</p>
                    </div>
                  ) : null}
                </div>
                <div className="ProductForm">
                  <button type="button" className="ProductForm__AddToCart Button Button--primary Button--full">
                ...
                <div className="FeaturedProduct__ViewWrapper">
                  <span className="Link Link--underline" style={{ opacity: 0.7 }}>
                    View product
                  </span>
                </div>
              </div>
