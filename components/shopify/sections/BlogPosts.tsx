...
export function BlogPosts({
  ...
  const buttonText = typeof settings.button_text === 'string' ? settings.button_text : '';
  const buttonLink =
    typeof settings.button_link === 'string' ? resolveShopifyLink(settings.button_link) : '#';

  return (
    <section className="Section Section--spacingNormal" id={`section-${id}`} data-section-type="article-list" data-section-id={id}>
            ...
            {[0, 1, 2].map((i) => (
              ...
              >
                <article className="ArticleItem">
                  <div className="ArticleItem__ImageWrapper AspectRatio" style={{ minHeight: 200, background: 'rgba(255,255,255,0.06)' }}>
                  ...
                  <div className="ArticleItem__Content">
                    {showCategory ? (
                      <span className="ArticleItem__Category Heading u-h6 Text--subdued">Journal</span>
                    ) : null}
                    ...
                    <p className="ArticleItem__Excerpt">
                      Article excerpts from Shopify Online Store blogging can be wired later. Placeholder cards keep the homepage layout.
                    </p>
                    <a href="/blog" className="ArticleItem__Link Link Link--underline">
                      Read more
                    </a>
                  </div>
                </article>
