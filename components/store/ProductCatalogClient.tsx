...
export function ProductCatalogClient({
  ...
  const filteredProducts = useMemo(() => {
    ...
    for (const [category, values] of Object.entries(filters)) {
      ...
      if (category === 'in-stock') {
        if (values.includes('true')) {
          result = result.filter(p => p.variants.some(v => v.inventoryQuantity === undefined || v.inventoryQuantity > 0));
        }
      } else if (category === 'price') {
        // Simple price filtering for demo
        const maxPrice = Math.max(...values.map(v => parseInt(v.split('-')[1] || '9999', 10)));
        result = result.filter(p => Math.min(...p.variants.map(v => v.price)) <= maxPrice);
      } else {
        // Tag-based filtering
        result = result.filter(p => p.tags.some(t => values.includes(t.toLowerCase())));
      }
    }
    ...
    if (sortBy === 'price-asc') {
      ...
      result.sort((a, b) => Math.max(...a.variants.map(v => v.price)) - Math.max(...b.variants.map(v => v.price)));
  ...
  const filterOptions = {
    ...
    engineered: [
      { id: 'material', label: 'Material', options: ['aluminum', 'steel', 'titanium'] },
      { id: 'category', label: 'Category', options: ['hardware', 'tools', 'accessories'] },
      { id: 'in-stock', label: 'Availability', options: ['true'] },
      { id: 'lead-time', label: 'Lead Time', options: ['in-stock', 'made-to-order'] },
    ],
    all: [
      { id: 'in-stock', label: 'Availability', options: ['true'] },
    ]
  ...
  const activeFilterCount = Object.values(filters).reduce((acc, val) => acc + val.length, 0);
  ...
  return (
    <div className="CollectionMain">
      <div className="CollectionToolbar">
        <div className="CollectionToolbar__Group">
          ...
          <div className="CollectionToolbar__Item CollectionToolbar__Item--filter">
            ...
            <div className="FilterList" style={{ display: 'flex', gap: '15px', marginTop: '10px', flexWrap: 'wrap' }}>
              {currentFilters.map(group => (
                <div key={group.id} className="FilterGroup">
                  <span className="Text--subdued" style={{ fontSize: 12, marginRight: 8 }}>{group.label}:</span>
                  <select 
                    className="Form__Input" 
                    style={{ padding: '4px 8px', fontSize: 13, display: 'inline-block', width: 'auto' }}
                    onChange={(e) => {
                      if (e.target.value) handleFilterToggle(group.id, e.target.value);
                      e.target.value = '';
                    }}
                  >
                    <option value="">Select...</option>
                    {group.options.map(opt => (
                      <option key={opt} value={opt}>{opt === 'true' ? 'In Stock' : opt}</option>
                    ))}
                  </select>
                </div>
              ))}
        ...
        <div className="CollectionToolbar__Group CollectionToolbar__Group--layout">
          <div className="CollectionToolbar__Item CollectionToolbar__Item--sort">
            <span className="Heading Text--subdued u-h6" style={{ marginRight: 8 }}>Sort</span>
            <select 
              className="Form__Input" 
              value={sortBy} 
          ...
          <div className="CollectionToolbar__Item CollectionToolbar__Item--layout">
            <div className="ProductViewToggle" role="group" aria-label="Product display layout" style={{ display: 'flex', gap: 10 }}>
              <button
                type="button"
      ...
      {activeFilterCount > 0 && (
        <div className="CollectionActiveFilters" style={{ display: 'flex', gap: 8, padding: '15px 0', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button className="Text--subdued Link Link--underline" style={{ fontSize: 12, marginLeft: 8 }} onClick={clearFilters}>
            Clear all
          </button>
        </div>
      )}
      ...
      {hasMore && (
        <div className="Pagination" style={{ textAlign: 'center', marginTop: 40 }}>
          <button 
