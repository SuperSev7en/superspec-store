  ...
  return (
    <div className="ClothingOverlay">
      ...
      <p className="Text--subdued" style={{ fontSize: 13, marginBottom: 20 }}>
        <strong>Fit Note:</strong> Model is 6&apos;1&quot;, wearing size L. True to size.
      </p>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <button onClick={() => setShowSizeGuide(true)} className="Link Link--underline Text--subdued" style={{ fontSize: 12 }}>Size Guide</button>
        </div>
        
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {product.variants.map(v => {
            // Try to extract size from title for label, otherwise use title
            const label = v.title === 'Default Title' ? 'One Size' : v.title;
            const isSoldOut = v.inventoryQuantity !== undefined && v.inventoryQuantity <= 0;
            ...
            return (
              <button
                ...
                style={{
                  ...
                  color: isSelected ? 'var(--background)' : (isSoldOut ? 'var(--text-light-color)' : 'var(--text-color)'),
      ...
      {stock > 0 && stock <= 5 && (
        <div style={{ padding: '8px 12px', background: 'rgba(249, 76, 67, 0.1)', color: 'var(--product-on-sale-color)', borderRadius: 4, fontSize: 13, fontWeight: 'bold', marginBottom: 20, display: 'inline-block' }}>
      ...
      <div style={{ borderTop: '1px solid var(--border-color)', borderBottom: '1px solid var(--border-color)', marginBottom: 20 }}>
        ...
        {showCare && (
          <div className="Rte Text--subdued" style={{ paddingBottom: 16, fontSize: 13 }}>
            <ul>
      ...
      {showSizeGuide && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'var(--background)', padding: 30, borderRadius: 8, width: '90%', maxWidth: 500, position: 'relative' }}>
            <button onClick={() => setShowSizeGuide(false)} style={{ position: 'absolute', top: 15, right: 15, fontSize: 20, background: 'none', border: 'none', cursor: 'pointer' }}>✕</button>
            <h3 className="Heading u-h3" style={{ marginBottom: 20 }}>Size Guide</h3>
              ...
              <tbody>
                {['S', 'M', 'L', 'XL', 'XXL'].map(size => (
                  <tr key={size} style={{ borderBottom: '1px solid var(--border-color)' }}>
                    <td style={{ padding: '10px 0', fontWeight: 'bold' }}>{size}</td>
                    <td>{size === 'S' ? '36"' : size === 'M' ? '40"' : size === 'L' ? '44"' : size === 'XL' ? '48"' : '52"'}</td>
                    <td>{size === 'S' ? '30"' : size === 'M' ? '34"' : size === 'L' ? '38"' : size === 'XL' ? '42"' : '46"'}</td>
                    <td>{size === 'S' ? '28"' : size === 'M' ? '29"' : size === 'L' ? '30"' : size === 'XL' ? '31"' : '32"'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
