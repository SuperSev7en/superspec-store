...
export function AdminProductsIndexClient({ products }: { products: Row[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((p) =>
      `${p.title} ${p.handle}`.toLowerCase().includes(query),
    );
  }, [products, q]);
          ...
          <table className="min-w-full divide-y divide-[#e1e3e5]">
            <thead className="bg-[#f4f6f8]">
              <tr>
                ...
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-[#616a75] uppercase tracking-wider">
                  Title
                </th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-[#616a75] uppercase tracking-wider">
                  Status
            ...
            <tbody className="bg-white divide-y divide-[#e1e3e5]">
              {filtered.length === 0 ? (
                <tr>
                  <td
                    colSpan={5}
                    className="px-4 py-12 text-center text-sm text-[#616a75]"
                  >
                    No products found.
                  </td>
                </tr>
                ...
                filtered.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-[#1a1a1a]">
                      <Link
                        className="hover:underline group-hover:text-[#005bd3]"
                        href={`/admin/products/${p.id}`}
                      >
                        {p.title}
                      </Link>
                      <div className="text-xs text-[#616a75] mt-0.5 font-normal">
                        /{p.handle}
                      </div>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-[#4a5568]">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${p.status === "active" ? "bg-[#aee9d1] text-[#006e52]" : "bg-[#e3e5e7] text-[#4a5568]"}`}
                      >
                        {p.status}
                      </span>
                    </td>
